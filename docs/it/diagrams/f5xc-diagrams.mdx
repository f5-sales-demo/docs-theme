...
description: >-
  Diagrammi dei casi d'uso di F5 Distributed Cloud per WAAP, difesa bot, rete
  multi-cloud, DDoS, DNS, sicurezza API e osservabilità con icone f5-brand.
sidebar:
  order: 22
i18n:
...
Diagrammi dei casi d'uso di F5 Distributed Cloud che illustrano architetture di sicurezza, rete e distribuzione delle applicazioni utilizzando il pacchetto di icone `f5-brand`.

## Protezione di app e API web

### Pipeline di ispezione WAAP multi-livello

Pipeline di ispezione WAAP multi-livello con firewall, protezione del codice applicativo e difesa bot prima di raggiungere l'applicazione.
...
### Architettura di sicurezza edge

Architettura di sicurezza edge con WAF, verifica tramite shield con segno di spunta e gruppi di protezione delle applicazioni su origini cloud.
...
### Protezione API con limitazione della frequenza

Pipeline di validazione delle richieste API con firewall, limitazione della frequenza e validazione dello schema prima di raggiungere gli endpoint API.
...
Rilevamento bot multi-stadio con sfida JavaScript, fingerprinting del dispositivo, analisi comportamentale e motore decisionale.
...
### Livelli di difesa bot

Architettura di difesa bot a più livelli con intelligence sulle credenziali, rilevamento bot e analisi della postura del dispositivo.
...
architecture-beta
  ...
  service acctsvc(carbon:security)[Account Service] in app
...
Pipeline di difesa lato client con verifica della postura del dispositivo, rilevamento bot su laptop e protezione Magecart.
...
### Connessione app multi-cloud

Connettività delle applicazioni multi-cloud tra AWS, Azure e GCP con fabric di distribuzione delle applicazioni centralizzato.
...
### Connessione di rete con site mesh

Connessione di rete multi-cloud con topologia site mesh e transit gateway che collega le regioni cloud.
...
### Distribuzione di app multi-cloud end-to-end

Distribuzione di app multi-cloud end-to-end con bilanciamento del carico globale, sicurezza e carichi di lavoro distribuiti.
...
### Architettura di scrubbing DDoS

Centro di scrubbing DDoS con protezione a livello di rete, scrubbing del sito e distribuzione del traffico pulito all'origine.
...
Flusso del traffico di attacco che mostra l'assorbimento e la mitigazione DDoS volumetrico all'edge prima di raggiungere l'origine.
...
### Protezione a più livelli CDN + DDoS + WAF

Protezione edge a più livelli che combina caching CDN, mitigazione DDoS e ispezione WAF in una pipeline unificata.
...
### GSLB basato su DNS con monitoraggio dello stato

Bilanciamento del carico del server globale basato su DNS con monitoraggio dello stato su endpoint multi-cloud.
...
### Architettura di gestione DNS

Infrastruttura di gestione DNS con bilanciamento del carico DNS e protezione Shield DNS su regioni cloud.
...
### Bilanciamento del carico DNS intelligente con failover

Bilanciamento del carico DNS intelligente con integrazione DNS cloud, routing basato sulle prestazioni e failover automatico.
...
## Sicurezza API e discovery

### Pipeline di discovery delle Shadow API

Pipeline di discovery delle shadow API che rileva le API sconosciute tramite analisi del traffico e gestione dell'inventario.
...
API gateway con autenticazione, limitazione della frequenza e validazione della sicurezza a protezione dei servizi API backend.
...
### Ciclo di vita API: dal discovery alla protezione

Pipeline del ciclo di vita API dalla discovery delle shadow API attraverso la catalogazione dell'inventario fino alla protezione attiva.
...
## Piattaforma e osservabilità

### App distribuite con NGINX One

Piattaforma applicativa distribuita con gestione NGINX One, carichi di lavoro Kubernetes e controllo centralizzato.
...
Pipeline di osservabilità che raccoglie metriche dalle applicazioni e produce insight, alert e dashboard.
...
### Vista completa della piattaforma

Vista completa della piattaforma F5 che collega sicurezza, rete e distribuzione delle applicazioni sotto un servizio unificato.
