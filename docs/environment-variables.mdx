...
| `DOCS_HOME` | `https://f5xc-salesdemos.github.io/f5xc-docs/` | Home page URL linked from the site title |
...
```js
const site = process.env.DOCS_SITE || 'https://f5xc-salesdemos.github.io';
```
...
Sets Astro's top-level `base` property. Required when deploying to a subdirectory (e.g., `https://example.com/my-repo/`).

### DOCS_HOME

```js
const docsHome = process.env.DOCS_HOME || 'https://f5xc-salesdemos.github.io/f5xc-docs/';
```
