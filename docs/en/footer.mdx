...
The footer override is registered by the Starlight plugin in `index.ts`. During the `config:setup` hook, it sets the `Footer` component override:
...
Content repositories do not need to register the footer manually — it is applied automatically when the theme plugin loads.
...
Edit the `href` attribute on the corresponding `<a>` tag in `components/Footer.astro` within the `@f5-sales-demo/docs-theme` package.
