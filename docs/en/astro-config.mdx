...
Content repositories do not maintain their own `astro.config.mjs`. Instead, they import a configuration factory from the `@f5-sales-demo/docs-theme` npm package that returns a complete Astro config with Starlight, eight bundled plugins, and all theme defaults.
...
| `logo` | `@f5-sales-demo/docs-theme/assets/github-avatar.png` | Sidebar logo (single src or light/dark pair) |
...
| `@f5-sales-demo/docs-theme` (self) | CSS injection, component overrides, route middleware |
...
These are registered in order inside `config.ts`. The theme plugin (`@f5-sales-demo/docs-theme`) is itself a Starlight plugin that runs in this list.
...
All paths use npm package specifiers (e.g., `@f5-sales-demo/docs-theme/styles/custom.css`) which resolve through the `exports` map in `package.json`.
