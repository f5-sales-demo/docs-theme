...
O tema lê as variáveis de ambiente em tempo de build a partir de `config.ts` e de componentes personalizados. Os repositórios de conteúdo definem essas variáveis em seus workflows do GitHub Actions para personalizar cada site sem modificar a configuração.
...
| `DOCS_SITE` | `https://f5-sales-demo.github.io` | URL base canônica do site implantado |
...
| `GITHUB_REPOSITORY` | _(string vazia)_ | Utilizada para construir o link social do GitHub e os links de edição |
...
Define a propriedade `base` de nível superior do Astro. Necessária ao implantar em um subdiretório (ex.: `https://example.com/my-repo/`).
...
Lida pelo componente personalizado `SiteTitle.astro`. Envolve o título do site no cabeçalho com um link para esta URL, permitindo que os usuários naveguem de volta para uma página inicial central de documentação. Útil quando vários repositórios de conteúdo compartilham o mesmo tema e você deseja um link "home" consistente em todos os sites.
...
Utilizada para gerar o ícone social do GitHub no cabeçalho do site e a URL base do link de edição. O GitHub Actions define essa variável automaticamente (ex.: `owner/repo`).
...
Um array codificado em JSON de objetos `{ title, url }` passados para o plugin `starlight-llms-txt`. Esses objetos aparecem como links adicionais no arquivo `llms.txt` gerado.
