...
设置 Astro 的顶级 `base` 属性。部署到子目录时需要设置此项（例如，`https://example.com/my-repo/`）。
...
用于在站点页眉中生成 GitHub 社交图标和编辑链接基础 URL。GitHub Actions 会自动设置此变量（例如，`owner/repo`）。
...
传递给 `starlight-llms-txt` 插件的 `{ title, url }` 对象的 JSON 编码数组。这些链接将作为附加链接出现在生成的 `llms.txt` 文件中。
